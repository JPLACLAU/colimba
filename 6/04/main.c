    ...
    while ((c = getword(word, MAXWORD)) != EOF) {
        ...
        for (int j = 0; j < wordCount - i - 1; j++) {
            if (count[j] < count[j + 1]) {
                ...
                int tempLineNumbers[MAXLINES];
                memcpy(tempLineNumbers, lineNumbers[j], sizeof(tempLineNumbers));
                memcpy(lineNumbers[j], lineNumbers[j + 1], sizeof(tempLineNumbers));
                memcpy(lineNumbers[j + 1], tempLineNumbers, sizeof(tempLineNumbers));
                char tempWord[MAXWORD];
                strcpy(tempWord, textWords[j]);
                strcpy(textWords[j], textWords[j + 1]);
                strcpy(textWords[j + 1], tempWord);
            }
        }
    }
